...
::GenerateData()
{
 ...
 // region->SetInput(imagen);
 // region->Update();  
 // m_Index=region->GetIndex();
 ...
 //  outputPtr->Allocate();
  //Te has pasado
 //  this->GraftOutput(region->GetOutput());
  /*OutputIterator it2 = OutputIterator(region->GetOutput(), Region2);
  OutputIterator out = OutputIterator(outputPtr, Region2);
  
  for ( it2.GoToBegin(), out.GoToBegin(); !it2.IsAtEnd(), !out.IsAtEnd(); ++it2, ++out )
    out.Set(it2.Get());*/
}
