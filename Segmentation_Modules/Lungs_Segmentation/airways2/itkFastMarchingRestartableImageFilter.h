/*=========================================================================
*
* This class implements a restartable, external controllable fast marching
* propagator which is better suited that to be used as a part of the
* thorax suite
*
* The implementation is heavly based on the original fast marching code from
* the Insight Toolkit
*
*=========================================================================*/
#ifndef __itkFastMarchingRestartableImageFilter_h
#define __itkFastMarchingRestartableImageFilter_h
...
namespace itk
{
/** \class FastMarchingRestartableImageFilter
...
class ITK_EXPORT FastMarchingRestartableImageFilter:
  public ImageToImageFilter< TSpeedImage, TLevelSet >
{
public:
  /** Standard class typdedefs. */
  typedef FastMarchingRestartableImageFilter Self;
...
* been processed. */
  ...
  /*
   * FIXME: This is new in the last itk code. What is it for?
   * It seems that is to limit the execution to a limited set of points.
   * This is good and can be used by us.
   */
...
* Trial points are represented as a VectorContainer of LevelSetNodes. */
  ...
  std::vector< itk::Point< unsigned int, SetDimension > > GetCurrentTrials();
  std::vector< itk::Point< unsigned int, SetDimension > > GetCurrentAlives();
...
protected:
  ...
  void AllocateOutput(LevelSetImageType *);
  void SetupPropagation(LevelSetImageType *);
...
private:
  FastMarchingRestartableImageFilter(const Self &); //purposely not implemented
  ...
  double latest_time; //Make it part of the body of the class so we can read it from the propagator
...
* the algorithm processes. */
  ...
  std::map<std::string, int> HashLabelMap;

  std::string idx2str(IndexType idx) {
	  std::stringstream ss;
	  for (int i=0;i<SetDimension;i++)
		  ss << idx.GetElement(i) << "_";

      return ss.str();
  }

  itk::Point< unsigned int, SetDimension > str2point(std::string  s_idx) {
	  std::string  token;
	  std::istringstream iss(s_idx);
	  itk::Point< unsigned int, SetDimension > idx;
	  idx.Fill(0);
	  int i=0;
	  while ( getline(iss, token, '_') ) {
		  idx.SetElement(i++, atoi(token.c_str()));
		  if (i > 2) break;
	  }
...
#include "itkFastMarchingRestartableImageFilter.cxx"
#endif
