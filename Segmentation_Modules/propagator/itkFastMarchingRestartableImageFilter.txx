...
template <class TLevelSet, class TSpeedImage>
std::vector< itk::Point< double, LevelSetTypeDefault<TLevelSet>::SetDimension > >
FastMarchingRestartableImageFilter<TLevelSet,TSpeedImage>
::GetTrials()
{

  std::vector< itk::Point< double, LevelSetTypeDefault<TLevelSet>::SetDimension > >  points;
  const unsigned int dimension = LevelSetTypeDefault<TLevelSet>::SetDimension;
  ...
  DataObject *output )
{
  ...
  if ( imgData )
    ...
    itkWarningMacro(<< "itk::FastMarchingRestartableImageFilter" <<
...
void
FastMarchingRestartableImageFilter<TLevelSet,TSpeedImage>
::RestartPropagation( LevelSetImageType * output )
{

   AxisNodeType node;
  
  // make sure the heap is empty and the map has no spurious trial point labels
  while ( !m_TrialHeap.empty() )
    {
        node = m_TrialHeap.top();
        m_TrialHeap.pop();
        m_LabelImage->SetPixel( node.GetIndex(), FarPoint );
    }
  ...
  if ( m_TrialPoints )
    ...
    for (; pointsIter != pointsEnd; ++pointsIter )
      ...
      PixelType outputPixel = node.GetValue();
      output->SetPixel( node.GetIndex(), outputPixel );
...
::GenerateData()
{
  ...
  if ( m_Restarted ) {
    std::cout << "Restarting propagation" << std::endl;
    this->RestartPropagation( output );
  } 
  else {
